...
uint32_t AssemblerParser::evaluateExpressionAt(int index) {
    if (index < 0 || (size_t)index >= tokens.size()) return 0;
...
void AssemblerParser::pass1() {
    while (peek().type != AssemblerTokenType::END_OF_FILE) {
        ...
        if (peek().type == AssemblerTokenType::IDENTIFIER) {
            ...
            } else if (match(AssemblerTokenType::EQUALS) || (peek().type == AssemblerTokenType::UNKNOWN && peek().value == "=")) {
                if (peek().type == AssemblerTokenType::UNKNOWN) advance();
                uint32_t val = evaluateExpressionAt(pos); advance();
        ...
        if (match(AssemblerTokenType::DIRECTIVE)) {
            ...
            if (stmt.dir.name == "var") {
                ...
                if (match(AssemblerTokenType::EQUALS) || (peek().type == AssemblerTokenType::UNKNOWN && peek().value == "=")) {
                    if (peek().type == AssemblerTokenType::UNKNOWN) advance();
            ...
            stmt.isInstruction = true;
            stmt.instr.mnemonic = tokens[pos-1].value;
            stmt.instr.address = pc;
            if (stmt.instr.mnemonic == "PROC") {
                std::string procName = advance().value; stmt.label = procName;
                ...
                while (match(AssemblerTokenType::COMMA)) {
                    if (match(AssemblerTokenType::HASH)) {
                        ...
                        stmt.instr.callArgs.push_back(std::string("#") + (v.type == AssemblerTokenType::HEX_LITERAL ? "$" : "") + v.value);
                        ...
                        const auto& v = advance();
                        stmt.instr.callArgs.push_back(p + "#" + (v.type == AssemblerTokenType::HEX_LITERAL ? "$" : "") + v.value);
                    } else {
                        const auto& v = advance();
                        stmt.instr.callArgs.push_back(std::string(v.type == AssemblerTokenType::HEX_LITERAL ? "$" : "") + v.value);
                    }
...
int AssemblerParser::calculateInstructionSize(const Instruction& instr) {
    ...
    if (instr.mnemonic == "BEQ" || instr.mnemonic == "BNE" || instr.mnemonic == "BRA") return 2;
    switch (instr.mode) {
...
uint8_t AssemblerParser::getOpcode(const std::string& m, AddressingMode mode) {
    if (m == "LDA") return (mode == AddressingMode::IMMEDIATE) ? 0xA9 : (mode == AddressingMode::ABSOLUTE_X ? 0xBD : (mode == AddressingMode::STACK_RELATIVE ? 0xD2 : 0xAD));
    if (m == "LDX") return (mode == AddressingMode::IMMEDIATE) ? 0xA2 : 0xAE;
    if (m == "LDY") return (mode == AddressingMode::IMMEDIATE) ? 0xA0 : 0xAC;
    if (m == "STA") return 0x8D; if (m == "JSR") return 0x20; if (m == "RTS") return 0x60;
        ...
        if (stmt.isInstruction) {
            if (stmt.instr.mnemonic == "PROC") continue;
                ...
                if (stmt.instr.mode == AddressingMode::IMMEDIATE || stmt.instr.mode == AddressingMode::STACK_RELATIVE) {
                    ...
                    binary.push_back(a & 0xFF); binary.push_back((a >> 8) & 0xFF);
                } else if (stmt.instr.mnemonic == "BEQ" || stmt.instr.mnemonic == "BRA") {
