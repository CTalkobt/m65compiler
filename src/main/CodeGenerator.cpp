...
void CodeGenerator::visit(StructDefinition& node) {
    ...
    for (const auto& member : node.members) {
        int size = 0;
        if (member.pointerLevel > 0) size = 2;
        else if (member.type == "char") size = 1;
        else if (member.type == "int") size = 2;
        else if (member.type.substr(0, 7) == "struct ") {
            std::string subName = member.type.substr(7);
            if (structs.count(subName)) size = structs[subName].totalSize;
            else throw std::runtime_error("Unknown struct type: " + subName);
        }
        info.members[member.name] = {member.type, member.pointerLevel, currentOffset};
        currentOffset += size;
    }
...
void CodeGenerator::visit(MemberAccess& node) {
    ...
    emitter->lda_ind_z(emitter->getZP(0), false);
    if (mInfo.pointerLevel > 0 || mInfo.type == "int" || (mInfo.type.substr(0, 7) == "struct " && structs[mInfo.type.substr(7)].totalSize > 1)) {
        emitter->pha();
        emitter->ldy_imm(1);
        std::stringstream ss;
