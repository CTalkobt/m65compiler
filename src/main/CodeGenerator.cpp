...
void CodeGenerator::visit(BinaryOperation& node) {
    ...
    if (node.op == "+") {
        ...
        else { emitter->sta_zp(0x02); emitter->stx_zp(0x03); emit("LDA 2, s"); emitter->clc(); emitter->adc_zp(0x02); emitter->pha(); emit("LDA 3, s"); emitter->adc_zp(0x03); emitter->tax(); emitter->pla(); }
    } else if (node.op == "-") {
        if (lhsSize == 1) { emitter->sta_zp(0x02); emit("LDA 1, s"); emitter->sec(); emitter->sbc_zp(0x02); emitter->ldx_imm(0); }
        else { emitter->sta_zp(0x02); emitter->stx_zp(0x03); emit("LDA 2, s"); emitter->sec(); emitter->sbc_zp(0x02); emitter->pha(); emit("LDA 3, s"); emitter->sbc_zp(0x03); emitter->tax(); emitter->pla(); }
    } else if (node.op == "&") {
        ...
        emitter->sta_zp(0x02); emitter->pop_ax(); std::string label = newLabel(); std::string end = newLabel();
        out << label << ":" << std::endl; emitter->lda_zp(0x02); emitter->beq(0x07); emitter->dec_zp(0x02); emitter->asl_a(); emit("ROL X"); emitter->bra(-11);
        out << end << ":" << std::endl;
