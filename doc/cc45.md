...
**Currently, processor flags (Carry, Zero, Negative, etc.) are NOT tracked.**
- Every operation that depends on a flag (like `ADC` or `BNE`) is preceded by an instruction that explicitly sets that flag to a known state (e.g., `CLC`, `SEC`, or `CMP`).
- This ensures correctness but leaves room for future optimization (e.g., skipping `CLC` if the carry is known to be clear).
