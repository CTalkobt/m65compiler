...
### Division by Zero
- **Compile Time**: Division by constant zero in expressions may be caught by the assembler if the compiler emits them as constant expressions.
- **Runtime**: Division by zero at runtime results in **undefined behavior**. The hardware math unit state becomes non-deterministic.
