...
### Jump Statements (`return`, `break`, `continue`, `asm`)
...
> **Note on Declaration Order**: Currently, type qualifiers (`volatile`, `auto`) must appear *before* the `unsigned` specifier. For example, `volatile unsigned int x;` is valid, but `unsigned volatile int x;` is not yet supported.
...
- **Control Flow**: `goto`.
- **Data Types**: `float`, `double`, `long`, `short`, `signed`, `unsigned` (explicit keyword), `enum`.
- **Operators**: `sizeof`.
- **Other**: `typedef`.
